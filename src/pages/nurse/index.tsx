...
// Debounce helper (как на странице поиска клиентов)
...
export const NursePage: React.FC = () => {
  ...
  const fetchAppointments = React.useCallback(async (selectedDate: string, forceRefetch = false) => {
    ...
    try {
      ...
      // Widen the range by +/- 1 day to account for timezone shifts (UTC vs Local)
      ...
      // Если не админ/супер-админ/регистратор, фильтруем по nurseId (для медсестер)
      if (!isAdmin() && !isRegistrator() && nurseId) {
        query = query.eq("doctor_id", nurseId);
      }
  ...
  const fetchRangeCounts = React.useCallback(async (forceRefetch = false) => {
    // Оптимизация 2: кэширование диапазона
    const d = new Date(date);
    const currentMonth = `${d.getFullYear()}-${d.getMonth() + 1}`; // Группируем кэш по месяцам для простоты
    ...
    try {
      const start = new Date(d);
      start.setDate(start.getDate() - 15); // Берем диапазон пошире
      const end = new Date(d);
      ...
      let query = supabase
        .from("AppointmentsAggregated")
        .select("appointment_at, performer_ids")
        .gte("appointment_at", startISO)
        .lte("appointment_at", endISO);

      if (!isAdmin() && !isRegistrator() && employeeId) {
        query = query.eq("doctor_id", employeeId);
      }
      ...
      if (!ctrl.signal.aborted) {
        ...
        (data || []).forEach(item => {
          // Filter by nurse if admin/superadmin
          if ((isAdmin() || isRegistrator()) && nurses.length > 0) {
            ...
            if (!isNurseInvolved) return; // Skip this appointment
          }
  ...
  }, [date, employeeId, isAdmin, isRegistrator, nurses]);
  ...
  React.useEffect(() => {
    ...
    const channel = supabase
      ...
      .on(
        ...
        (payload) => {
          console.log("Nurse Realtime update received (Appointments):", payload);
      ...
      .on(
        ...
        (payload) => {
          console.log("Nurse Realtime update received (AppointmentServices):", payload);
          delete appointmentsCache.current[date];
          countsCache.current = {};
          fetchAppointments(date, true);
          fetchRangeCounts(true);
      ...
      .on(
        ...
        (payload) => {
          console.log("Nurse Realtime update received (MedicalConclusions):", payload);
          delete appointmentsCache.current[date];
          countsCache.current = {};
          fetchAppointments(date, true);
          fetchRangeCounts(true);
  ...
  const resetFilters = () => {
    ...
    setStatus({
      ...
      "Клиент здесь": true,
      "Со скидкой": true,
      "Клиент не пришел": false, // Default hidden to reduce clutter? Or true? User didn't specify, false is safer for main view.
      "Отменено": true
    });
    setOnlyNight(false);
  };
