...
import { createShift, updateShift, deleteShift, Shift } from "../../services/shifts";
...
const WorkShiftsPage: React.FC = () => {
    ...
    const handleEditSuccess = async (data: any) => {
        ...
        try {
            ...
            const result = await updateShift(shiftToEdit.id, {
                employes_id: data.employes_id,
                clock_in: new Date(clockInISO).toISOString(),
                clock_out: clockOutISO ? new Date(clockOutISO).toISOString() : undefined,
                is_night_shift: data.is_night_shift,
                shift_date: data.startDate,
                start_time: data.start_time,
                end_time: data.end_time || undefined
            });

            if (!result) throw new Error("Failed to update shift");
    ...
    const handleDeleteShift = async (shift: Shift) => {
        ...
        try {
            ...
            if (!success) throw new Error("Failed to delete shift");
                                                        ...
                                                        <TableRow hover>
                                                            ...
                                                            {isAdmin() && (
                                                                <TableCell align="right">
                                                                    ...
                                                                    {isSuperAdmin() && (
                                                                        <IconButton size="small" color="error" onClick={() => handleDeleteShift(shift as any)} title="Удалить">
                                                    ...
                                                    <CardContent sx={{ p: 1.5, '&:last-child': { pb: 1.5 } }}>
                                                        <Stack direction="row" alignItems="flex-start" justifyContent="space-between">
                                                            ...
                                                            <Stack direction="row" alignItems="center" spacing={0.5} ml={1} sx={{ flexShrink: 0 }}>
                                                                ...
                                                                {isSuperAdmin() && (
                                                                    <IconButton size="small" color="error" onClick={() => handleDeleteShift(shift as any)}>
                                                                        <DeleteOutline fontSize="small" />
                                                                    </IconButton>
                                                                )}
                                                            </Stack>
