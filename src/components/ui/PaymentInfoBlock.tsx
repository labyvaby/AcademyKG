...
export const PaymentInfoBlock: React.FC<PaymentInfoBlockProps> = ({
  ...
  const getStatusConfig = () => {
    ...
    if (isPaid) {
      const isCardOnly = card > 0 && cash === 0;
      return {
        label: "Оплачено",
        color: isCardOnly ? ("info" as const) : ("success" as const),
        icon: <CheckCircleOutline fontSize="small" />,
        bgColor: isCardOnly ? theme.palette.info.main : theme.palette.success.main,
        lightBg: isCardOnly ? alpha(theme.palette.info.main, 0.08) : alpha(theme.palette.success.main, 0.08),
      };
    }
    if (isPartiallyPaid) {
      return {
          ...
          <Stack spacing={2}>
            ...
            <Stack spacing={1.5}>
              ...
              {discountPercent !== undefined && discountPercent > 0 && (
                <Stack direction="row" justifyContent="space-between" alignItems="center">
                  <Stack direction="row" spacing={1} alignItems="center">
                    <Box sx={{ p: 0.5, borderRadius: 1, bgcolor: alpha(theme.palette.secondary.main, 0.1), display: 'flex' }}>
                      <Typography sx={{ fontSize: 14, fontWeight: 800, color: 'secondary.main' }}>%</Typography>
                    </Box>
