...
type AddPayrollDrawerProps = {
  open: boolean;
  onClose: () => void;
  onCreated?: (record: PayrollTransaction) => void;
};

const defaultValues: PayrollFormValues = {
  employee_id: null,
  kind: null,
  affects_month: dayjs().format("YYYY-MM"),
  name: "",
  cash_amount: 0,
  cashless_amount: 0,
  comment: ""
...
export const AddPayrollDrawer: React.FC<AddPayrollDrawerProps> = ({ open, onClose, onCreated }) => {
  ...
  React.useEffect(() => {
    if (!open) {
      setValues({ ...defaultValues, affects_month: dayjs().format("YYYY-MM") });
      setBusy(false);
      setTouched(false);
    }
  }, [open]);
  ...
  const handleSubmit = async () => {
    ...
    setBusy(true);
    try {
      const created = await PayrollTransactionsService.create({
...
export default AddPayrollDrawer;
