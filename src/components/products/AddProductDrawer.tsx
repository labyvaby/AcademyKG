...
export const AddProductDrawer: React.FC<AddProductDrawerProps> = ({
    ...
    const handleSubmit = async () => {
        setTouched(true);
        if (!branchId) {
            notify?.({ type: "error", message: t("products.selectBranchToCreate") });
            return;
        }
        if (!values.name.trim()) {
        ...
        try {
            ...
            notify?.({ type: "success", message: t("products.productAdded") });
            onClose();
        } catch (e: unknown) {
            console.error("Create product failed:", e);
            notify?.({ type: "error", message: t("products.createError") });
        } finally {
            setBusy(false);
        }
    };
                                ...
                                >
                                    ...
                                    <Box sx={{ flex: 1 }}>
                                        <Typography variant="body2">
                                            {photoFile ? photoFile.name : t("products.uploadPhoto")}
                                        </Typography>
