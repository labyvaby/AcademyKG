...
// Логотип Profigram временно отключён (файл отсутствует)
const logoImg = "";
...
export const ProfigramInviteDialog: React.FC<ProfigramInviteDialogProps> = ({
    ...
    const [note, setNote] = useState(`Здравствуйте, ${patientName || "уважаемый пациент"}! Это ваш специалист.\n\nПредлагаю подключиться к сервису Profigram. Отправляю приглашение.`);
    ...
    useEffect(() => {
        if (open) {
            ...
            setNote(`Здравствуйте, ${patientName || "уважаемый пациент"}! Это ваш специалист.\n\nПредлагаю подключиться к сервису Profigram. Отправляю приглашение.`);
        }
    }, [open, patientName]);
