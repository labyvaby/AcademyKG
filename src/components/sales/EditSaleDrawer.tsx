    ...
    useEffect(() => {
        if (open && sale) {
            ...
            if (match) {
                ...
                // Everything before the payment info is the user comment
                const splitIndex = rawComment.indexOf("Оплата:");
                setUserComment(splitIndex > -1 ? rawComment.substring(0, splitIndex).trim() : "");
            } else {
                // Legacy or simple comment
                setUserComment(rawComment);
                // Default to 0? Or maybe try to infer paid status?
                // For now, default to 0 to force re-entry or just show 0
                setCash("");
                setCard("");
        ...
        const fetchPatients = async () => {
            ...
            if (data) {
                setPatients(data.map(p => ({
                    id: p["ID"] || p.id,
                    label: p.full_name || p["ФИО пациента"] || "Без имени"
                })));
            }
        };
            ...
            >
                ...
                <Stack spacing={0.5}>
                    ...
                    <Autocomplete
                        ...
                        renderInput={(params) => <TextField {...params} placeholder="Выберите пациента" />}
