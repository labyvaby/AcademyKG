...
const ScheduleCalendar = React.forwardRef<ScheduleCalendarHandle, ScheduleCalendarProps>((props, ref) => {
  ...
  const filteredShifts = useMemo(() => {
    ...
    // 1. Фильтр для обычных пользователей (не админов/регистраторов): видят только свои смены
    if (!canManage && employeeId) {
      result = result.filter(s => s.employes_id === employeeId);
    }

    // 2. Фильтр по выбранной роли (Admin/Registrator Only)
    // Если админ/регистратор выбрал "Все", показываем всех.
    ...
    return result;
  }, [shifts, selectedSpec, isAdmin, isRegistrator, canManage, employeeId, selectedRole]);
    ...
    <Paper elevation={3} sx={{ p: 2, mt: 2 }}>
      ...
      <Stack direction="row" alignItems="center" spacing={2} sx={{ mb: 1.5, overflow: 'hidden' }}>
        ...
        >
          ...
          {canManage && availableSpecs.length > 0 && (
            <Box sx={{ width: "1px", height: 24, bgcolor: 'divider', mx: 0.5 }} />
          )}

          {/* Specialization Filters */}
          {canManage && availableSpecs.map((spec) => (
            <Chip
              key={spec}
              label={spec}
              size="small"
