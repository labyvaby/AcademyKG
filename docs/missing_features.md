...
| 5 | `PATCH /appointment-groups/{id}/` не принимает `maxParticipants` | 🟡 Средний |
| 6 | `GET /appointment-groups/` нет фильтра по тренеру | 🟡 Средний |
| 7 | `POST /client-schedules/bulk/` неверный формат ответа | 🟡 Средний |
| 8 | Двойное списание баллов (фронт + возможно бэкенд) | 🟢 Низкий |
